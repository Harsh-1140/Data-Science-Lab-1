...
url <- 'https://www.espncricinfo.com/series/ipl-2020-21-1210595/match-schedule-fixtures-and-results'
...
for (i in seq_along(matchlinks)) {
  ...
  if (length(tables) >= 3) {
    ...
    if (ncol(bowling_table1) >= 11 && ncol(bowling_table2) >= 11) {
      ...
      team1_bowling_df <- mutate(team1_bowling_df, Match_ID = 202000 + i)
      team2_bowling_df <- mutate(team2_bowling_df, Match_ID = 202000 + i)
...
combined_bowling_data_2020 <- bind_rows(all_matches_data)

combined_bowling_data_2020 <- combined_bowling_data_2020 %>% select(Match_ID,Team,everything())
